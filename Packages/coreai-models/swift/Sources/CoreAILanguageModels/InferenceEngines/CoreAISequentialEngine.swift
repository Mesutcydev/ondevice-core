...
/// - **2–4 states**: KV cache pair plus optional persistent hybrid-model states
///
...
public final class CoreAISequentialEngine: InferenceEngine, @unchecked Sendable {
    ...
    // State handlers own allocation, growth, and reset behavior.
    ...
    init(
        ...
        // Hybrid models may expose fixed recurrent state in addition to KV cache.
        ...
        if let additionalStates {
            CLILogger.log(
                "Additional persistent states: \(additionalStates.stateNames.joined(separator: ", "))"
            )
        }
    ...
    private func processTokenBatch(_ tokens: ArraySlice<Int32>) async throws -> [LogitsScalarType] {
        ...
        // MutableViews keeps lifetime dependencies on its inout bindings, so
        // bind the supported state counts explicitly in this call scope.
        var kv0 = kvCache[stateIndex: 0]
        var kv1 = kvCache[stateIndex: 1]
        if var additionalStates {
            switch additionalStates.stateCount {
            case 1:
                var state0 = additionalStates[stateIndex: 0]
                var states = InferenceFunction.MutableViews()
                states.insert(&kv0.array, for: kv0.name)
                states.insert(&kv1.array, for: kv1.name)
                states.insert(&state0.array, for: state0.name)
                _ = try await function.run(
                    inputs: [inputIdsName: inputIdsArray, positionIdsName: positionIds],
                    states: consume states,
                    outputViews: consume outputViews
                )
                additionalStates[stateIndex: 0] = state0
            case 2:
                var state0 = additionalStates[stateIndex: 0]
                var state1 = additionalStates[stateIndex: 1]
                var states = InferenceFunction.MutableViews()
                states.insert(&kv0.array, for: kv0.name)
                states.insert(&kv1.array, for: kv1.name)
                states.insert(&state0.array, for: state0.name)
                states.insert(&state1.array, for: state1.name)
                _ = try await function.run(
                    inputs: [inputIdsName: inputIdsArray, positionIdsName: positionIds],
                    states: consume states,
                    outputViews: consume outputViews
                ...
                additionalStates[stateIndex: 0] = state0
                additionalStates[stateIndex: 1] = state1
            default:
                preconditionFailure(
                    "Unsupported additional state count \(additionalStates.stateCount)"
                )
            }
