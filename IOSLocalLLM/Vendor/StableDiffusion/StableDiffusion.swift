...
/// Quantization is lazy in MLX. Materialize each component immediately after
/// converting it so source weights and conversion scratch buffers are released
/// before the next large SDXL component is loaded.
private func quantizeAndMaterializeTextEncoder(_ model: CLIPTextModel) {
    quantize(model: model, filter: { _, module in module is Linear })
    eval(model)
    Memory.clearCache()
}

private func quantizeAndMaterializeUNet(_ model: UNetModel) {
    quantize(model: model, groupSize: 32, bits: 8)
    eval(model)
    Memory.clearCache()
}
...
enum SDModelContainerError: LocalizedError {
...
public actor SDModelContainer<M> {
    ...
    static public func createTextToImageGenerator(
        ...
        if let model = try configuration.textToImageGenerator(configuration: loadConfiguration) {
            ...
            throw SDModelContainerError.unableToCreate(configuration.id, "TextToImageGenerator")
        }
    }
    ...
    static public func createImageToImageGenerator(
        ...
        if let model = try configuration.imageToImageGenerator(configuration: loadConfiguration) {
            ...
            throw SDModelContainerError.unableToCreate(configuration.id, "ImageToImageGenerator")
        }
    }
    ...
    public func perform<R>(_ action: @Sendable (M) throws -> R) throws -> R {
        switch state {
        case .discarded:
            throw SDModelContainerError.modelDiscarded
        case .loaded(let m):
            try action(m)
        }
    }
    ...
    public func performTwoStage<R1, R2>(
        ...
        if conserveMemory {
            ...
            // The model is gone, but MLX's default cache is intentionally
            // unbounded and otherwise keeps its multi-GB buffers alive through
            // the VAE decode. Stage one has fully evaluated its output, so this
            // is a safe synchronization point to reclaim those buffers.
            Memory.clearCache()
        }
...
open class StableDiffusion {
    ...
    internal init(
        ...
        let loadedUNet =
            try unet ?? loadUnet(hub: hub, configuration: configuration, dType: dType)
        if quantizeOnLoad {
            quantizeAndMaterializeUNet(loadedUNet)
        }
        self.unet = loadedUNet

        let loadedTextEncoder =
            try textEncoder ?? loadTextEncoder(
                hub: hub, configuration: configuration, dType: dType)
        if quantizeOnLoad {
            quantizeAndMaterializeTextEncoder(loadedTextEncoder)
        }
        self.textEncoder = loadedTextEncoder
...
open class StableDiffusionXL: StableDiffusion, TextToImageGenerator, ImageToImageGenerator {
    ...
    public init(
        ...
        let loadedTextEncoder2 = try loadTextEncoder(
            hub: hub, configuration: configuration, configKey: .textEncoderConfig2,
            weightsKey: .textEncoderWeights2, dType: dType)
        if quantizeOnLoad {
            quantizeAndMaterializeTextEncoder(loadedTextEncoder2)
        }
        self.textEncoder2 = loadedTextEncoder2
