...
class UNetMidBlock: Module {
    // NOTE: non-numeric keys are required here. This is a Module subclass (not a
    // tuple), so it presents to MLX as `.value(.module)`. Numeric keys ("0"/"1"/"2")
    // make the checkpoint weights unflatten into an `.array`, and MLX's update has no
    // `(.value(.module), .array)` case -> "not compatible" load failure. Named keys
    // make the weights unflatten into a `.dictionary`, which the matcher handles.
    //
    // The attention/resnet1 are optional so distilled SDXL checkpoints whose
    // `mid_block_type` is "UNetMidBlock2D" (a plain mid block with NO cross
    // attention — e.g. Segmind Vega / SSD-1B) load and run instead of feeding
    // SDXL conditioning into a mismatched cross-attention matmul and crashing.
    @ModuleInfo(key: "resnet0") var resnet0: ResnetBlock2D
    @ModuleInfo(key: "attention0") var attention0: Transformer2D?
    @ModuleInfo(key: "resnet1") var resnet1: ResnetBlock2D?

    init(configuration: UNetConfiguration, channelsLast: Int, channels0: Int) {
        self._resnet0.wrappedValue = ResnetBlock2D(
            inputChannels: channelsLast,
            outputChannels: channelsLast,
            groupCount: configuration.normNumGroups,
            timeEmbedChannels: channels0 * 4
        )
        if configuration.midBlockType != "UNetMidBlock2D" {
            self._attention0.wrappedValue = Transformer2D(
                inputChannels: channelsLast,
            ...
            self._resnet1.wrappedValue = ResnetBlock2D(
                inputChannels: channelsLast,
                outputChannels: channelsLast,
                groupCount: configuration.normNumGroups,
                timeEmbedChannels: channels0 * 4
            ...
            self._attention0.wrappedValue = nil
            self._resnet1.wrappedValue = nil
        }
    }
}
...
class UNetModel: Module {
    ...
    @ModuleInfo(key: "mid_blocks") var midBlocks: UNetMidBlock
    ...
    init(configuration: UNetConfiguration) {
        ...
        self._midBlocks.wrappedValue = UNetMidBlock(
    ...
    func callAsFunction(
        ...
        // run the middle part of the unet. The attention/resnet1 are absent for
        // plain ("UNetMidBlock2D") distilled checkpoints like Segmind Vega/SSD-1B.
        x = midBlocks.resnet0(x, timeEmbedding: temb)
        if let attention0 = midBlocks.attention0 {
            x = attention0(
                x, encoderX: encoderX, attentionMask: attentionMask,
                encoderAttentionMask: encoderAttentionMask)
        }
        if let resnet1 = midBlocks.resnet1 {
            x = resnet1(x, timeEmbedding: temb)
        }
